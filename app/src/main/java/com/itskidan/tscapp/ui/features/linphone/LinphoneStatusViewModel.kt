...
@HiltViewModel
class LinphoneStatusViewModel @Inject constructor(
    ...
    private val _uiState = MutableStateFlow(LinphoneUiState())
    val uiState: MutableStateFlow<LinphoneUiState> get() = _uiState
    ...
    fun onActivateService() {
        viewModelScope.launch {
            ...
            Timber.Forest.tag("MyLog").d("[onActivateServiceClicked] FcmToken: $token")
            registerSIPAccount()
        }
    }
    ...
    fun onInfoClick() {
        viewModelScope.launch {
            ...
            accountList.forEach { account ->
                ...
                Timber.Forest.tag("MyLog").d("[onInfoClick] account $identity")
            }
        }
    }
